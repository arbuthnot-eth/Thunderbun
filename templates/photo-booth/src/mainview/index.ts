...
class PhotoBooth {
	...
	constructor() {
		...
		this.cameraModeBtn = document.getElementById(
			"cameraModeBtn",
		) as HTMLButtonElement;
		this.screenModeBtn = document.getElementById(
			"screenModeBtn",
		) as HTMLButtonElement;
		this.startCameraBtn = document.getElementById(
			"startCameraBtn",
		) as HTMLButtonElement;
		...
		this.captureBtnText = this.captureBtn.querySelector(
			".capture-btn-text",
		) as HTMLElement;
		this.cameraIcon = this.captureBtn.querySelector(
			".capture-icon-camera",
		) as HTMLElement;
		this.screenIcon = this.captureBtn.querySelector(
			".capture-icon-screen",
		) as HTMLElement;
	...
	private initializeEventListeners() {
		...
		document
			...
			?.addEventListener("click", () => this.saveCurrentPhoto());
		document
			.getElementById("deleteBtn")
			?.addEventListener("click", () => this.deleteCurrentPhoto());
	...
	private setMode(mode: CaptureMode) {
		...
		this.captureBtnText.textContent =
			mode === "camera" ? "Take Photo" : "Take Screenshot";
		...
		const placeholder = this.video.parentElement?.querySelector(
		...
		if (mode === "camera") {
			...
			this.setStatus(
				"Screen capture mode - tests getDisplayMedia browser API",
				false,
			);
		...
		try {
			...
			if (
				...
				try {
					...
					this.video.srcObject = this.stream;
					this.setStatus(
						"Screen capture active - ready to take screenshots",
						true,
					);
					this.captureBtn.disabled = false;
					this.selectScreenBtn.style.display = "none";
					...
					throw new Error(
						`Screen capture failed: ${(permissionError as Error).message}`,
					);
				}
			} else {
				...
				throw new Error(
					"getDisplayMedia API is not available in this browser. This may be due to:\n• WKWebView limitations\n• Browser version\n• Security restrictions\n• Platform limitations",
				);
			}
		} catch (error) {
	...
	private async captureScreenshot() {
		try {
			if (this.stream) {
				...
				throw new Error(
					"No screen capture stream available. Make sure to select a screen first.",
				);
			}
		} catch (error) {
	...
	private deleteCurrentPhoto() {
		...
		// Remove from DOM
		const photoElement = this.gallery.querySelector(
			`[data-photo-id="${this.currentPhotoId}"]`,
		);
		if (photoElement) {
			photoElement.remove();
		}
